...
默认数据存在容器里的 SQLite（`/data/app.db`）。如果你想把数据放到自己的 **MySQL** 上，进「设置」页最上方的「数据库」一栏，**分别填**主机、端口、用户名、密码、数据库名，点「测试并保存」即可：

- 系统会**先测连接、再自动建表**，成功才切换，**立即生效、无需重启**；失败会原样退回 SQLite 并提示原因。
- **主机地址留空 = 切回本地 SQLite。**
- 密码里的特殊字符（`@ : / . #` 等）会自动转义，不用手动处理。
- 需要 **MySQL 8.0.13+**（用到窗口函数 / CTE / `DEFAULT (expr)`），建议库字符集用 `utf8mb4`。
- 连接信息**不写进数据库**（否则就循环依赖了），而是存到 `data/instance.json`；也可以用环境变量 `DATABASE_URL` 预先设定整串 `mysql://...`。
...
TCGdex 的日文本地化也只覆盖了一小部分（比如「リザードン / 喷火龙」TCGdex 只有约 25 张，而集换社等市场有 99+）。所以本项目同样内置了一份**官方日文卡库**：

- **内置离线卡库（默认开启，免配置）**：打包了约 2.2 万张日文单卡，数据抓自宝可梦官方日文卡查（社区项目 [type-null/PTCG-database](https://github.com/type-null/PTCG-database)，见 `scripts/build_ja_cards.py` 与 `app/data/ja_cards.json`）。这些卡 ID 形如 `jp:3021`，与 TCGdex 的日文卡共用「日文」筛选，使得「喷火龙」的日文结果能达到 80+ 张。订阅后没有自动价格（官方无公开价格 API），可在详情页用「手动记录」按日元(JPY)录价。
- 卡图默认走官方 `pokemon-card.com`，加载慢或被墙时可在「设置」页改 `日文卡图地址前缀`。
